...
setwd("~/SMC-HET-Paper1/smc_het_eval")
...
penalties <- c("abs", "sq")
method.names <- c("orig",  "pseudoV",   
                  "sqrt", "sym_pseudoV", "pearson", "aupr", "mcc")

ordering.names <- c("Aggregate", "Paul", "Nathan", "Dave", "Adriana", "Peter", "Quaid")
method.is.good.greater <- list("orig"=T,  "pseudoV"=F,  
                               "sqrt"=T, "sym_pseudoV"=F, "pearson"=T, "aupr"=T, "mcc"=T)
